...
class LivraisonC {
    ...
    $search_result = configAhmed::filterTable($sql);
...
function recupererLivraisonClient($idCommande){
	$sql="  SELECT etat,designation,nomLivreur,concat(date(dateConfirmerLiv),' à ',hour(dateConfirmerLiv),':',minute(dateConfirmerLiv)) as dateConfirmerLiv,CodeLiv FROM livraison where idCommande='$idCommande' ";
		$db = configAhmed::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
