...
if (isset($_SESSION["login_in"])) {
    if (!empty($_SESSION["ID"])) {
        ...
        $listeProduits=$Produits->afficherProduitss($id);

    
        $connect = mysqli_connect('localhost','root','','bweb');
        $query = "SELECT nom ,quantite as number FROM produit GROUP BY nom";
        $result = mysqli_query($connect, $query);
        $queryy = "SELECT reference ,quantite as number FROM quantite_vendu GROUP BY reference";
    ...
    <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>  
           ...
           function drawChart2()
           {
               var data = google.visualization.arrayToDataTable([
                   ...
                   {
                       if($row["number"] > 0)
                           echo "['".$row["reference"]."', ".$row["number"]."],";
                       else
                           echo "['".$row["reference"]."', 0],";
                   }
                   ?>
               ]);
               var options = {
            ...
            <div class="row">
                ...
                <!--<div class="col-md-12">--> <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    ...
                    <!--<div class="table-responsive">--> <div class="card">
                                <h7 class="card-header" style="font-weight : bolder">Liste des produits</h7> <!-- font weight-->
...
<!-- !CADRE -->
            ...
            <!-- CADRE -->
            <div class="card">
                <h7 class="card-header" style="font-weight : bolder">Produits les plus vendus</h7> <!-- font weight-->

                <div class="card-body"  > <!-- class="table-responsive" -->
                    <div style="width:100%;">
                        <h2 align="center">  </h2>
                        <div align="center" id="piechart2" style="width: 100%; height: 600px"></div>
                    </div>
                </div>
            </div>
            <!-- !CADRE -->
...
//alert(id+nom,+referennce+categorie+quantite_max+quantite_min,montant_max,montant_min);
            ...
            {//alert('a'+montant_min+'b'+montant_max);
                if(check_int && check_qmin && check_qmax && check_mmin && check_mmax) {
                    $.ajax({
                        url: "Recherche.php",
                        method: "POST",
                        data: {
