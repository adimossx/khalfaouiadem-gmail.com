...
class ReclamationC {
	...
	function afficherReclamationFront($idcc){
		//$sql="SElECT * From employe e inner join formationphp.employe a on e.cin= a.cin";
		/*$sql="SElECT * From reclamation where $idcc";
		$db = config9::getConnexion();
		try{
            $liste=$db->query($sql);
            return $liste;
		...
		$sth = $dbh->prepare('SElECT * From reclamation where id_client=:idc');
		$sth->bindValue(':idc', $idcc);
		$sth->execute();
		$liste= $sth->fetchAll();
		return $liste;
	}
